...
//    private ProgressBar progressBar;
    ...
    public UpdateAppManager(Context context, boolean manualCheckUpdate) {
        ...
        UPDATE_SERVER_ADDRESS = context.getString(R.string.serverIP);
        ...
        // 下载路径
        spec = UPDATE_SERVER_ADDRESS + context.getString(R.string.downloadUrl);

        // 版本路径
        versionUrl = UPDATE_SERVER_ADDRESS + context.getString(R.string.getLatestApkVersionURL);
...
//        params.put("code", 1); // 1: 渔民app  2: 安装app
        ...
        client.get(versionUrl, null, new JsonHttpResponseHandler() {
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                try {
                    ...
                    if (response.getBoolean("Flag")) {
                        ...
                        serverVersion = apkVersion.getInt("Version");
                        serverVersionName = apkVersion.getString("VersionName");
                        UpgradeTime = apkVersion.getString("UpgradeTime");
                        UpgradeTime = getFormatTime(UpgradeTime);
                        Remark = apkVersion.getString("Remark");
                        forceToUpdate = apkVersion.getBoolean("Is_update");
                        ...
                        if (serverVersion > currentVersion) {
                            ...
                            showNoticeDialog();
                            editor.putInt("serverVersionCode", serverVersion);
                            editor.putString("newVersionRemark", Remark);
                        } else if (serverVersion == currentVersion) {
                            editor.putString("oldVersionRemark", system.getString("newVersionRemark", ""));
                            //手动检测更新
                            if (manualCheckUpdate) {
                                showNoUpdateDialog();
                            }
