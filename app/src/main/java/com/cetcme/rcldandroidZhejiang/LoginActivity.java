        ...
        if (ships != null) {
            ...
            try {
                for (int i = 0; i < ships.size(); i++) {
                    ...
                    shipJson.put("name", ships.get(i).name);
                    shipJson.put("number", ships.get(i).number);
                    shipJson.put("latitude", ships.get(i).latitude);
                    shipJson.put("longitude", ships.get(i).longitude);
                    shipJson.put("deviceInstall", ships.get(i).deviceInstall);

                    shipsJson.put(shipJson);
                }
                Log.i("Main", shipsJson.toString());
                editor.putString("ships", shipsJson.toString());
                editor.apply();
            } catch (JSONException e) {
                e.printStackTrace();
            }
